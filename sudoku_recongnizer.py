...
def mainfunction(the_file):

    image = None
    # loading the image
    image = Image.open(the_file).convert('RGB')
    ...
    thresh = cv.adaptiveThreshold(
        cv_image, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 199, 5)
    ...
    if mode == 'thresh':
        ...
        # img = cv.cvtColor(thresh, cv.COLOR_RGB2BGR)
    else:
    ...
    for i in range(9):
        for j in range(9):
            ...
            grid[i][j] = this.crop(
                (twenty, twenty, box_size-twenty, box_size-twenty))
